...
export default function YouTubeDescriptionGenerator() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/generate-description', {
        ...
        body: JSON.stringify({ videoTitle, videoKeywords }),
      });
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `${videoTitle.slice(0, 30)}-description.txt`;
  ...
  const handleCopy = async () => {
    try {
      await navigator.clipboard.writeText(description.join('\n'));
      setCopied(true);
    } catch (err) {
    ...
    <div className="flex gap-8">
      <div className="flex-1">
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100">
          <div className="p-8">
            <div className="mb-8">
              <h2 className="text-2xl font-semibold text-gray-900 mb-2">Generate Your YouTube Description</h2>
              <p className="text-gray-600">Enter your video details below to generate an SEO-optimized description</p>
            </div>
            
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label htmlFor="videoTitle" className="block text-sm text-gray-700 mb-2">
                  Video Title
                </label>
                <input
                  type="text"
              ...
              <div>
                <label htmlFor="videoKeywords" className="block text-sm text-gray-700 mb-2">
                ...
                <input
                  ...
                  className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:border-red-500 focus:ring-2 focus:ring-red-500/20 transition"
                  placeholder="gaming, tutorial, minecraft, lets play"
                  required
                />
              </div>
              ...
              <div className="p-8">
                ...
                <div className="space-y-2">
                  {description.map((line, index) => (
                    ...
                    >
                      <span className="text-gray-900 whitespace-pre-wrap">{line}</span>
                    </div>
                  ))}
                </div>
              </div>
