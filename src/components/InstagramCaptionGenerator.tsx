...
export default function InstagramCaptionGenerator() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('Failed to generate Instagram caption. Please try again.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="flex gap-8">
      <div className="flex-1">
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100">
          <div className="p-8">
            <div className="mb-8">
              <h2 className="text-2xl font-semibold text-gray-900 mb-2">Generate Your Instagram Caption</h2>
              <p className="text-gray-600">Enter your image details and preferences to generate engaging Instagram captions</p>
            </div>
            
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label htmlFor="imageDescription" className="block text-sm text-gray-700 mb-2">
                  Image Description
                </label>
                <textarea
                  id="imageDescription"
                  ...
                  className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:border-pink-500 focus:ring-2 focus:ring-pink-500/20 transition"
              ...
              <div>
                <label htmlFor="keywords" className="block text-sm text-gray-700 mb-2">
                ...
                <input
                  ...
                  className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:border-pink-500 focus:ring-2 focus:ring-pink-500/20 transition"
                  placeholder="lifestyle, fashion, summer"
                  required
                />
              </div>
              ...
              <button
                ...
                className="relative w-full bg-gradient-to-r from-pink-500 via-pink-600 to-orange-400 text-white py-4 px-6 rounded-xl hover:opacity-90 transition disabled:opacity-50 disabled:cursor-not-allowed shadow-md"
              >
                {loading ? (
              ...
              <div className="p-8">
                ...
                <div className="space-y-6">
                  {captions.map((caption, index) => (
                    ...
                    >
                      <div className="text-gray-900 whitespace-pre-wrap">{caption}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {categories.map((category, index) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center text-pink-500 group-hover:bg-pink-500/5 transition">
                    {getCategoryIcon(category.icon)}
                  </div>
                  <span className="text-gray-900">{category.name}</span>
                </div>
                <span className="text-sm text-pink-500 bg-white px-3 py-1 rounded-lg">
                  {category.count}
                </span>
              </a>
            ))}
