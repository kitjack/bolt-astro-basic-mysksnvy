...
export default function FacebookPostGenerator() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('Failed to generate Facebook post. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCopy = async () => {
    try {
      ...
      console.error('Failed to copy posts:', err);
    }
  };
    ...
    <div className="flex gap-8">
      <div className="flex-1">
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100">
          <div className="p-8">
            <div className="mb-8">
              <h2 className="text-2xl font-semibold text-gray-900 mb-2">Generate Your Facebook Post</h2>
              <p className="text-gray-600">Enter your topic and preferences to create engaging Facebook posts</p>
            </div>
            
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label htmlFor="topic" className="block text-sm text-gray-700 mb-2">
                  Post Topic or Main Message
                </label>
                <textarea
                  id="topic"
                  value={topic}
                  onChange={(e) => setTopic(e.target.value)}
                  className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:border-facebook focus:ring-2 focus:ring-facebook/20 transition"
              ...
              <div>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="w-5 h-5 rounded text-facebook focus:ring-facebook/20"
                  />
              ...
              <button
                ...
                className="relative w-full bg-facebook text-white py-4 px-6 rounded-xl hover:bg-facebook/90 transition disabled:opacity-50 disabled:cursor-not-allowed shadow-md"
              >
                {loading ? (
            ...
            {categories.map((category, index) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center text-facebook group-hover:bg-facebook/5 transition">
                    {getCategoryIcon(category.icon)}
                  </div>
                  <span className="text-gray-900">{category.name}</span>
                </div>
                <span className="text-sm text-facebook bg-white px-3 py-1 rounded-lg">
                  {category.count}
                </span>
              </a>
            ))}
